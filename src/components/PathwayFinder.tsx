...
export default function PathwayFinder() {
  ...
  const downloadGuidance = async () => {
    ...
    write('Kimatu Pathway Finder Results', { bold: true, size: 16, color: [37, 99, 235] });
    ...
    y += 12;
    write('Kimatu Analytics · https://kimatu.company', { size: 9, color: [120, 120, 120] });
    doc.save(`kimatu-pathway-${(career?.id || 'results').replace(/\s+/g, '-')}.pdf`);
  };
        ...
        {step === 3 && !showResults && (
          <div className="max-w-3xl mx-auto">
            ...
            <div className="bg-amber-500/10 rounded-2xl p-6 border border-amber-500/30 mb-8 text-center">
              ...
              <p className="text-gray-300 text-sm">Pay KSh 50 to view your results and see how your performance compares to the requirements for each pathway.</p>
            </div>
              ...
              <div className="flex items-center gap-3">
                {canSeeResults() && !paymentUnlocked && (
                  ...
                  >
                    ...
                    {paying ? 'Opening payment…' : 'Pay KSH 50'}
                  </button>
                )}
                <button
                  onClick={() => {
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-[#2563EB] text-white rounded-xl hover:bg-blue-700 transition-colors font-medium disabled:opacity-50"
                  title={paymentUnlocked ? '' : 'Please pay KSH 50 to unlock results'}
                >
        ...
        {step === 3 && showResults && !paymentUnlocked && (
          <div className="max-w-3xl mx-auto">
            <div className="bg-gradient-to-br from-amber-500/20 to-orange-500/20 rounded-2xl p-8 border border-amber-500/30 mb-8 text-center">
              ...
              <p className="text-gray-300 mb-4">Please complete the KSH 50 payment to unlock your pathway results.</p>
              <button
                onClick={handlePay}
                disabled={paying}
