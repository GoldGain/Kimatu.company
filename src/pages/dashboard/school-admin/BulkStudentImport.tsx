...
export default function BulkStudentImport() {
  ...
  const importStudents = async () => {
    ...
    setResults([]);
    try {
      const { data: existing } = await supabaseUntyped.from('students').select('admission_number, student_email').eq('school_id', user.schoolId);
      const existingAdmissions = new Set((existing || []).map((s: any) => String(s.admission_number || '').trim().toLowerCase()));
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        try {
          if (existingAdmissions.has(admissionKey)) throw new Error('admission number already exists in this school');
          ...
          completed.push({ row: index + 2, admission_number: admission, name, email, password: '', status: 'failed', message: error?.message || 'Import failed' });
        }
  ...
  const downloadCredentials = () => {
    ...
    downloadCsv('kimatu-student-login-credentials.csv', [
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <section className="lg:col-span-2 bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-4">
          <div className="flex flex-wrap gap-2 items-center justify-between">
            ...
            <button type="button" onClick={() => downloadCsv('kimatu-student-import-template.csv', [TEMPLATE_HEADERS,           ['ADM001', 'John', '', 'Kamau', classes[0]?.name || 'Grade 1', '', 'Male', '', '', 'Kenyan', '', '', 'day', '', 'CBE', '', '', '', '']])} className="text-sm font-bold text-blue-700 hover:underline flex items-center gap-1"><Download size={15} /> Download template</button>
          </div>
        ...
        <section className="bg-blue-50 border border-blue-200 rounded-2xl p-5 space-y-3">
          ...
          <p className="text-sm text-blue-800">Each learner is checked for duplicate admission numbers and emails before account creation. Existing learners are not overwritten.</p>
