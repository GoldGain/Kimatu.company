      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-6 text-white">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-blue-100 text-sm">{student.classes?.name} &middot; Assessment #: {student.admission_number}</p>
          </div>
          {allMarks.length > 0 && (
            <div className="text-right">
              <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border font-bold text-sm ${PERFORMANCE_COLORS[overallLevel]}`}>
                ...
                {overallLevel} &mdash; {PERFORMANCE_LABELS[overallLevel]}
              </div>
                ...
                {homework.slice(0, 20).map((h, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{h.homework?.subjects?.name} &middot; {new Date(h.submitted_at).toLocaleDateString()}</p>
                    </div>
