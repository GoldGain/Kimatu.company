/**
 * Kimatu System AI — powerful in-app assistant for every portal.
 * - Page-aware guidance
...
function guideForPath(path: string): string {
  ...
  return (
    match
      ? PAGE_GUIDES[match]
      : 'You are on a Kimatu Analytics page. Ask how to complete a task on this screen, where a feature lives, or what each section is for. I will explain the page and give step-by-step guidance.'
  );
}
...
function buildSystemPrompt(ctx: AiContext, liveNotes: string): string {
  ...
  return [
    'You are Kimatu Copilot, a calm in-app guide for Kimatu Analytics (Kenya CBE and 8-4-4).',
...
export async function askKimatuAssistant(
  question: string,
  ctx: AiContext,
