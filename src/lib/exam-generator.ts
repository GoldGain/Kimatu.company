...
function addPageHeader(doc: jsPDF, paper: ExamPaper, continuation = false): number {
  ...
  doc.setFontSize(13);
  doc.text((paper.school_name || 'KIMATU ANALYTICS').toUpperCase(), 105, 10, { align: 'center' });
  doc.setFontSize(10);
...
function addFooter(doc: jsPDF): void {
  ...
  for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
    ...
    doc.text('Generated securely by Kimatu Analytics Curriculum Navigator', 14, 291);
